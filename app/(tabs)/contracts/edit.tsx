...
export default function EditContractScreen() {
    ...
    const fetchContract = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setValue(contractData.value?.toString() || "");
          setCurrency(contractData.currency || "VNĐ");
          setTerm(contractData.term?.toString() || "");
          setStatus(contractData.status || "Chờ duyệt");
          
          // Set dates
          if (contractData.signedDate) {
            setSignedDate(contractData.signedDate.toDate());
          }
          if (contractData.expriryDate) {
            setExpiryDate(contractData.expriryDate.toDate());
          }
        } else {
          Alert.alert("Lỗi", "Không tìm thấy hợp đồng");
          router.back();
        }
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateDoc(docRef, updatedContract);
      
      console.log("Đã cập nhật hợp đồng:", contract.id);
      ...
      Alert.alert("Lỗi", "Không thể cập nhật hợp đồng. Vui lòng thử lại.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <Text style={styles.title}>Chỉnh sửa hợp đồng</Text>
